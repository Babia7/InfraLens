...
const ArchitecturalSchematic: React.FC<{ mode: 'LEGACY' | 'ARISTA' }> = ({ mode }) => {
  return (
    <div className="h-64 w-full bg-surface-muted border border-border rounded-2xl relative overflow-hidden flex items-center justify-center p-8">
       ...
       {mode === 'LEGACY' ? (
          <div className="relative w-full h-full flex items-center justify-center">
             ...
             <div className="flex gap-16 items-center">
                {[1, 2, 3].map(i => (
                   <div key={i} className="relative group">
                      <div className="w-16 h-16 rounded-xl bg-card-bg border border-border flex flex-col items-center justify-center gap-1 shadow-2xl relative z-10">
                         <Box size={24} className="text-secondary" />
                         <span className="text-[8px] font-mono text-secondary uppercase tracking-tighter">P{i}</span>
                      </div>
                      {/* MESSAGING ARROWS */}
                      {i < 3 && (
             ...
             <div className="absolute bottom-4 text-[10px] font-mono text-secondary uppercase tracking-widest italic">Distributed Messaging Architecture</div>
          </div>
       ) : (
          <div className="relative w-full h-full flex items-center justify-center">
             {/* SYSDB CORE */}
             <div className="relative">
                ...
                {[0, 72, 144, 216, 288].map((angle, i) => (
                   <div 
                      key={i} 
                      className="absolute w-8 h-8 rounded-lg bg-card-bg border border-border flex items-center justify-center transition-all duration-700 hover:scale-110"
             ...
             <div className="absolute bottom-4 text-[10px] font-mono text-secondary uppercase tracking-widest italic">State-Based SysDB Architecture</div>
          </div>
       )}
    </div>
  );
...
export const ArchitecturalGapAnalysis: React.FC<ArchitecturalGapAnalysisProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-page-bg text-primary font-sans flex flex-col overflow-hidden selection:bg-rose-500/30">
      ...
      <header className="h-16 border-b border-border flex items-center justify-between px-8 bg-page-bg shrink-0 z-50">
        <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <span className="tool-label">Reasoning Kernel v4.1</span>
                </div>
            </div>
        </div>
        <div className="flex items-center gap-4">
           <div className="bg-card-bg border border-border px-3 py-1 rounded-full flex items-center gap-2">
              ...
              <span className="tool-label">SysDB Verified</span>
           </div>
        </div>
      </header>
        ...
        <section className="w-full md:w-[450px] lg:w-[500px] bg-page-bg border-r border-border flex flex-col shrink-0 relative overflow-y-auto">
           ...
           <div className="p-8 md:p-12 relative z-10 space-y-12">
              <header className="space-y-4">
                 <h2 className="text-4xl font-serif font-bold text-primary tracking-tighter">Failure Modes</h2>
                 <p className="text-sm text-secondary leading-relaxed">Select a scenario to deconstruct the architectural response gap between legacy messaging and Arista state-streaming.</p>
              </header>
              ...
              <div className="grid grid-cols-1 gap-4">
                 {COMPARISON_SCENARIOS.map((scenario) => (
                    <button 
                       ...
                       className="p-6 bg-card-bg border border-border rounded-3xl text-left transition-all hover:border-rose-500/50 hover:bg-card-bg/80 group flex items-center justify-between"
                    >
                       <div className="flex items-center gap-6">
                          <div className="w-12 h-12 rounded-2xl bg-surface-muted border border-border flex items-center justify-center text-secondary group-hover:text-rose-400 transition-colors">
                             <scenario.icon size={24} />
                          </div>
                          <span className="font-bold text-primary group-hover:text-rose-400 transition-colors">{scenario.label}</span>
                       </div>
                       <ChevronRight size={18} className="text-secondary group-hover:text-rose-400" />
                    </button>
                 ))}
              </div>
            ...
            <div className="max-w-4xl mx-auto space-y-12 animate-fade-in relative z-10">
               {!result ? (
                  <div className="h-[60vh] flex flex-col items-center justify-center text-center opacity-20">
                     ...
                     <h3 className="text-3xl font-serif italic text-secondary">Select a scenario for gap synthesis</h3>
                  </div>
               ) : result && (
                  <div className="space-y-16 pb-20">
                     <header className="space-y-4">
                        ...
                        <h2 className="text-5xl md:text-7xl font-serif font-black tracking-tighter text-primary">{result.scenario}</h2>
                     </header>

                     {/* SCHEMATIC COMPARISON */}
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                           <h3 className="text-[10px] font-bold uppercase tracking-widest text-secondary">Legacy Paradigm</h3>
                        ...
                        <div className="space-y-4">
                           <h3 className="text-[10px] font-bold uppercase tracking-widest text-secondary">Arista Paradigm</h3>
                     ...
                     {/* BUSINESS METRIC */}
                     <section className="bg-card-bg border border-border rounded-[2.5rem] p-10 relative overflow-hidden group shadow-2xl">
                        ...
                        <div className="flex items-center gap-4 mb-8">
                           ...
                           <h3 className="text-2xl font-serif font-bold text-primary">Business Value Projection</h3>
                        </div>
                        <div className="text-4xl font-serif font-bold text-primary mb-4 tracking-tight">
                           {result.businessMetric}
                        </div>
                        <p className="text-lg text-secondary font-light leading-relaxed italic border-l-2 border-rose-500/30 pl-8">
                     ...
                     <section className="space-y-6">
                        <h3 className="text-xs font-bold uppercase tracking-widest text-secondary">CTO Battlefield Points</h3>
                        <div className="grid grid-cols-1 gap-4">
                           {result.bulletPoints.map((point, i) => (
                              <div key={i} className="group p-6 bg-card-bg border border-border rounded-2xl flex items-center justify-between hover:border-border transition-all cursor-default">
                                 <div className="flex items-center gap-6">
