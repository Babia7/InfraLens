      ...
      <header className="h-16 border-b border-border flex items-center justify-between px-8 bg-page-bg shrink-0 z-50">
        <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <span className="tool-label">Lossless Fabric v2.1</span>
                </div>
            </div>
        </div>
        <div className="flex items-center gap-4">
           <div className="bg-card-bg border border-border px-3 py-1 rounded-full flex items-center gap-2">
              ...
              <span className="tool-label">Lossless State: OK</span>
           </div>
           {!isDefaults && (
             <div className="bg-card-bg border border-border px-3 py-1 rounded-full flex items-center gap-2">
           ...
           {onNavigate && (
             <button
               ...
               className="px-3 py-1 rounded-full border border-border bg-card-bg text-[10px] font-bold uppercase tracking-widest text-secondary hover:text-primary hover:border-border transition-colors flex items-center gap-2"
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <section className="w-full md:w-[400px] bg-page-bg border-r border-border flex flex-col shrink-0 relative overflow-y-auto">
           ...
           <div className="p-8 md:p-10 relative z-10 space-y-12">
              <header className="space-y-4">
                 <h2 className="text-4xl font-serif font-bold text-primary tracking-tighter">Fabric Spec</h2>
                 <p className="text-sm text-secondary leading-relaxed">Define storage requirements to synthesize a validated Ethernet Storage Fabric.</p>
              </header>

              <div className="space-y-6">
                 <div className="space-y-3">
                    <label className="tool-label">Storage Protocol</label>
                    <div className="grid grid-cols-1 gap-2">
                       {['NVMe-oF RoCE v2', 'NVMe-TCP', 'iSCSI'].map(val => (
                          <button 
                             key={val}
                             onClick={() => setProtocol(val)}
                             className={`p-4 rounded-xl border text-xs font-bold transition-all text-left flex justify-between items-center ${protocol === val ? 'bg-blue-900/20 border-blue-500 text-blue-400 shadow-lg shadow-blue-500/10' : 'bg-card-bg border-border text-secondary hover:border-border'}`}
                          >
                             {val}
                 ...
                 <div className="space-y-3">
                    <label className="tool-label">Design Preset</label>
                    <select
                      value={designPreset}
                      onChange={(e) => setDesignPreset(e.target.value as DesignPreset)}
                      className="w-full bg-card-bg border border-border rounded-xl p-4 text-sm outline-none focus:border-blue-500 transition-colors"
                    >
                 ...
                 <div className="space-y-3">
                    <label className="tool-label">Port Speed</label>
                    <select
                      value={portSpeed}
                 ...
                 <div className="space-y-3">
                    <label className="tool-label">Host Count</label>
                    <input
                    ...
                    <div className="flex justify-between text-2xl font-serif font-bold text-primary">
                       <span>{hostCount}</span>
                       <span className="text-xs font-mono text-secondary self-end">Hosts</span>
                    </div>
                 </div>

                 <div className="space-y-3">
                    <label className="tool-label">Oversubscription Target</label>
                    <select
                 ...
                 <div className="space-y-3">
                    <label className="tool-label">Traffic Mix</label>
                    <select
                      value={trafficMix}
                 ...
                 <div className="space-y-3">
                    <label className="tool-label">Latency Tier</label>
                    <select
                      value={latencyTier}
                 ...
                 <div className="space-y-3">
                    <label className="tool-label">Redundancy</label>
                    <select
                      value={redundancy}
              ...
              <div className="p-6 bg-blue-950/10 border border-blue-500/20 rounded-2xl">
                 <div className="flex items-start gap-4">
                    ...
                    <p className="text-[11px] text-secondary leading-relaxed italic">
                       Calculated Buffer Requirement: <span className="text-primary font-bold">{latencyTier === 'Ultra-low' ? '32GB (Deep Buffer)' : '16GB (Universal)'}</span> based on congestion profile.
                    </p>
                 </div>
              </div>

              <div className="p-6 bg-card-bg border border-border rounded-2xl space-y-3">
                 <div className="flex items-center justify-between gap-3">
                    <div className="text-[10px] font-mono uppercase tracking-widest text-secondary">NVMe-oF Protocol Lab Notes</div>
                    {onNavigate && (
                      <button
                        onClick={() => {
                          try {
                        ...
                        className="text-[10px] font-mono uppercase tracking-[0.3em] text-blue-400 hover:text-primary transition-colors"
                      >
                        Protocol Lab →
                      </button>
                    )}
                 ...
                 <div className="text-[10px] font-mono uppercase tracking-widest text-secondary">Protocol guardrails</div>
                 <ul className="text-xs text-secondary space-y-2 list-disc list-inside">
        ...
        <section className="flex-1 overflow-y-auto bg-surface-muted p-8 md:p-16 relative">
            ...
            <div className="max-w-5xl mx-auto space-y-12 animate-fade-in relative z-10">
               <section className="bg-card-bg border border-border rounded-[2.5rem] p-10 shadow-2xl">
                  <div className="flex items-center gap-3 mb-8">
                     <div className="w-10 h-10 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400">
                        <Network size={20} />
                     </div>
                     <div>
                        <span className="text-[10px] font-mono text-secondary uppercase tracking-[0.3em] block">Arista Fabric Sizing</span>
                        <h3 className="text-2xl font-serif font-bold text-primary">Port Map & Platform</h3>
                     </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Assumptions</span>
                     ...
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Fabric Targets</span>
                        <div className="mt-2 space-y-1 text-xs text-secondary">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Topology</span>
                        <p className="text-lg text-primary font-semibold mt-2">{plannerOutput.topology}</p>
                        <p className="text-xs text-secondary mt-2">{plannerOutput.fabricSplit}</p>
                     </div>
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Platform Class</span>
                        <p className="text-lg text-primary font-semibold mt-2">{plannerOutput.platform.model}</p>
                        <p className="text-xs text-secondary mt-2">{plannerOutput.platform.rationale}</p>
                     </div>
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Port Plan</span>
                        <p className="text-lg text-primary font-semibold mt-2">{plannerOutput.portPlan.portSpeedGb}G x {plannerOutput.portPlan.hostPortsPerNode}</p>
                        <p className="text-xs text-secondary mt-2">Per host bandwidth: {plannerOutput.portPlan.perHostBandwidthGb}G</p>
                     </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Scalable Units</span>
                        <p className="text-xl text-primary font-semibold mt-2">{plannerOutput.portPlan.suCount}</p>
                        <p className="text-xs text-secondary mt-2">{hostCount} total hosts</p>
                     </div>
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Leaf Ports</span>
                        <p className="text-xl text-primary font-semibold mt-2">{plannerOutput.portPlan.leafPortsRequired}</p>
                        <p className="text-xs text-secondary mt-2">Spine uplinks: {plannerOutput.portPlan.spineUplinksRequired}</p>
                     </div>
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Aggregate Bandwidth</span>
                        <p className="text-xl text-primary font-semibold mt-2">{plannerOutput.portPlan.totalHostBandwidthGb}G</p>
                        <p className="text-xs text-secondary mt-2">Oversubscription: {plannerOutput.portPlan.oversubscription}</p>
                     </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Lossless Profile</span>
                        <p className="text-xs text-secondary mt-2">{plannerOutput.losslessProfile.pfc}</p>
                        <p className="text-xs text-secondary mt-2">{plannerOutput.losslessProfile.ecn}</p>
                        <p className="text-xs text-secondary mt-2">{plannerOutput.losslessProfile.mtu}</p>
                     </div>
                     <div className="p-4 bg-surface-muted border border-border rounded-2xl">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-secondary">Validation Flags</span>
                        {plannerOutput.validationFlags.length === 0 ? (
                          <p className="text-xs text-secondary mt-2">No flags for this configuration.</p>
                        ) : (
                          <ul className="text-xs text-secondary mt-2 space-y-2 list-disc list-inside">
               ...
               {!strategy && !loading ? (
                  <div className="h-[40vh] flex flex-col items-center justify-center text-center opacity-20">
                     <div className="relative mb-8">
                        <div className="w-24 h-24 border-2 border-dashed border-border rounded-[2rem] flex items-center justify-center animate-spin-slower">
                           <HardDrive size={40} className="text-secondary" />
                        </div>
                        <Database size={20} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-secondary" />
                     </div>
                     <h3 className="text-2xl font-serif italic text-secondary">Synthesize strategy for narrative output</h3>
                  </div>
               ) : loading ? (
                  <div className="h-[40vh] flex flex-col items-center justify-center space-y-8">
                     ...
                     <div className="text-center">
                        <h4 className="font-bold text-primary uppercase tracking-[0.3em] text-sm">Synthesizing Storage Strategy</h4>
                        <p className="text-secondary text-xs mt-2 font-mono">[Reasoning-Engine: Calibrating Lossless Profile...]</p>
                     </div>
                  </div>
               ) : strategy && (
                  <div className="space-y-16 pb-20">
                     <header className="space-y-4">
                        ...
                        <h2 className="text-5xl md:text-7xl font-serif font-black tracking-tighter text-primary">Lossless Design</h2>
                     </header>

                     {/* TOPOLOGY DASHBOARD */}
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="p-8 bg-card-bg border border-border rounded-[2.5rem] space-y-6 shadow-2xl group hover:border-blue-500/30 transition-all">
                        ...
                        <div className="p-8 bg-card-bg border border-border rounded-[2.5rem] space-y-6 shadow-2xl group hover:border-blue-500/30 transition-all">
                           ...
                           <p className="text-sm text-secondary leading-relaxed">Optimal ASIC selection for {protocol} buffer requirements.</p>
                        </div>
                     </div>
                    ...
                    <section className="bg-card-bg border border-border rounded-[3rem] p-12 relative overflow-hidden group shadow-2xl">
                       ...
                       <div className="flex justify-end mb-6">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-full border border-blue-500/30 bg-blue-500/10 text-[10px] font-mono uppercase tracking-[0.3em] text-blue-300 hover:text-primary hover:border-blue-400 transition"
                          >
                            Copy runbook
                          </button>
                       </div>
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                           <div className="space-y-2">
                              <span className="text-[10px] font-mono text-secondary uppercase">PFC (Priority Flow Control)</span>
                              <p className="text-lg text-primary font-medium">{strategy.losslessConfig.pfc}</p>
                           </div>
                           <div className="space-y-2">
                              <span className="text-[10px] font-mono text-secondary uppercase">ECN (Congestion Notification)</span>
                              <p className="text-lg text-primary font-medium">{strategy.losslessConfig.ecn}</p>
                           </div>
                           <div className="space-y-2">
                              <span className="text-[10px] font-mono text-secondary uppercase">Buffer Strategy</span>
                              <p className="text-lg text-primary font-medium">{strategy.losslessConfig.bufferStrategy}</p>
                           </div>
                        </div>
                     </section>
                     ...
                     <section className="space-y-6">
                        <div className="p-10 bg-gradient-to-br from-card-bg to-surface-muted border border-border rounded-[3rem] shadow-2xl">
                           ...
                           <p className="text-3xl text-primary font-serif leading-tight italic">
                     ...
                     {/* DEPLOYMENT NOTES */}
                     <section className="space-y-6">
                        <h3 className="text-xs font-bold uppercase tracking-widest text-secondary">Critical Deployment Notes</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           {strategy.deploymentNotes.map((note, i) => (
                              <div key={i} className="p-6 bg-card-bg border border-border rounded-3xl group hover:border-blue-500/30 transition-all flex items-start gap-4">
                     ...
                     <div className="p-12 bg-blue-950/20 border border-blue-500/20 rounded-[3rem] text-center">
                        <h3 className="text-2xl font-serif font-bold text-primary mb-4">Export Validated Blueprint</h3>
                        <p className="text-secondary mb-8 max-w-xl mx-auto leading-relaxed">Generate the EOS CLI and AVD parameters for this lossless storage fabric.</p>
