  ...
  return (
    <div className={`group relative flex items-center ${className}`}>
      ...
      <div className={`absolute ${posClasses[side]} z-[100] pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-200`}>
        <div className="bg-border text-primary text-[10px] font-medium py-1.5 px-3 rounded-lg border border-border shadow-xl whitespace-nowrap">
          {text}
          {/* Tiny arrow */}
          <div className={`absolute w-2 h-2 bg-border border-border transform rotate-45 ${
  ...
  return (
    <div className="min-h-screen bg-page-bg text-primary font-sans flex flex-col overflow-hidden selection:bg-emerald-500/30">
           ...
           <div className="hidden md:flex gap-1">
              {LAB_MODULES.map((m, i) => (
                <Tooltip key={m.id} text={m.title} side="bottom">
                    <div className={`h-1 w-12 rounded-full transition-all duration-500 cursor-help ${i === activeModuleIdx ? 'bg-emerald-500' : 'bg-border'}`}></div>
                </Tooltip>
              ))}
           </div>
        </div>
      ...
      <main className="flex-1 flex overflow-hidden min-h-0">
        ...
        {/* MODULE SIDEBAR */}
        <aside className="w-64 border-r border-border bg-page-bg/50 hidden lg:flex flex-col">
            <div className="p-6">
                <h3 className="text-[10px] font-bold text-secondary uppercase tracking-[0.3em] mb-6">Execution Sequence</h3>
                    ...
                    {LAB_MODULES.map((module, idx) => (
                        <Tooltip key={module.id} text={module.summary} side="right" className="w-full">
                            <button 
                                ...
                                className={`w-full text-left p-4 rounded-xl transition-all border flex items-center gap-4 ${activeModuleIdx === idx ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-transparent border-transparent text-secondary hover:text-primary'}`}
                            >
                                <module.icon size={18} />
            ...
            <div className="mt-auto p-6 border-t border-border">
                <div className="p-4 bg-card-bg/50 rounded-xl border border-border">
                    <p className="text-[10px] text-secondary leading-relaxed italic">"Arista Advanced Cloud Training (ACT) emphasizes that EOS is a set of Linux processes."</p>
                </div>
            </div>
        </aside>
        ...
        <div className="flex-1 overflow-y-auto relative p-8 md:p-16 flex flex-col min-h-0">
            ...
            <div className="max-w-3xl animate-fade-in">
                <header className="mb-12">
                    ...
                    <h2 className="text-5xl font-serif font-bold text-primary mb-6 leading-tight">{activeStep.heading}</h2>
                ...
                <div className="space-y-12 pb-20">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="p-6 bg-surface-muted border border-border rounded-2xl relative group overflow-hidden shadow-2xl">
                            ...
                            <code className="text-emerald-400 font-mono text-lg block">
                                {activeStep.command.split('\n').map((line, i) => (
                                    <div key={i} className="flex gap-3 mb-1 last:mb-0">
                                        <span className="text-secondary select-none shrink-0">$</span>
                                        <span className="break-all">{line}</span>
                                    </div>
                                ))}
                            </code>
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <span className="text-[8px] font-mono text-secondary uppercase">Interactive_Shell_Active</span>
                            </div>
                        </div>
                        <p className="text-sm text-secondary leading-relaxed pl-4 border-l border-border italic">
                            {activeStep.explanation}
                        </p>
                    </div>

                    {/* INSIGHT CARD */}
                    <div className="p-8 bg-card-bg border border-border rounded-[2rem] relative overflow-hidden shadow-xl">
                        ...
                        <p className="text-lg text-primary leading-relaxed font-serif">
                            {activeStep.insight}
                        </p>
                    </div>
                </div>
                ...
                {/* NAVIGATION FOOTER */}
                <div className="mt-auto border-t border-border pt-10 flex justify-between items-center">
                    <div className="flex gap-1">
                        {activeModule.content.map((_, i) => (
                            <div key={i} className={`h-1 w-8 rounded-full transition-all ${i === activeStepIdx ? 'bg-emerald-500' : 'bg-border'}`}></div>
                        ))}
                    </div>
                    <button 
                        onClick={handleNext}
        ...
        {/* METRICS PANEL */}
        <div className="w-80 border-l border-border bg-page-bg/30 hidden xl:flex flex-col p-8 gap-8 overflow-y-auto">
            <section>
                ...
                <div className="space-y-4">
                    <div className="p-4 bg-card-bg/50 rounded-xl border border-border group relative">
                        <span className="text-[10px] text-secondary uppercase block mb-1">Architecture</span>
                        <span className="text-sm font-bold">Linux-Based (Fedora)</span>
                    </div>
                    <div className="p-4 bg-card-bg/50 rounded-xl border border-border">
                        <span className="text-[10px] text-secondary uppercase block mb-1">Kernel Version</span>
                ...
                <div className="space-y-3">
                    {[
                        ...
                        <Tooltip key={item.label} text={item.tip} side="left">
                            <div className="flex justify-between text-[10px] font-mono border-b border-border pb-2 cursor-help">
                                <span className="text-secondary border-b border-dashed border-border">{item.label}</span>
