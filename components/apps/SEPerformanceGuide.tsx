...
export interface SEPerformanceGuideProps {
  onBack: () => void;
  startAbout?: boolean;
}
        ...
        >
            <div className="max-w-xl p-8 relative">
                ...
                <div className="flex items-center gap-4 mb-8">
                    <span className="font-mono text-[10px] text-secondary uppercase tracking-widest border border-border px-2 py-1 rounded bg-card-bg">
                ...
                <h2 className="font-serif font-bold text-primary mb-8 leading-[1.1] tracking-tight" style={{ fontSize: 'clamp(2.5rem, 5vw, 4.5rem)' }}>
                    {title}
                </h2>
                <div className="space-y-10">
                    <p className="text-secondary font-sans leading-relaxed max-w-lg" style={{ fontSize: 'clamp(1rem, 1.2vw, 1.25rem)' }}>
                        {description}
                    </p>
                    <div className="relative pl-6 border-l-2 border-indigo-500/30 group hover:border-indigo-500 transition-colors">
                        ...
                        <p className="text-primary font-medium leading-relaxed" style={{ fontSize: 'clamp(0.9rem, 1.1vw, 1.125rem)' }}>
                            {actionText}
                        </p>
                    </div>
                </div>
...
export const SEPerformanceGuide: React.FC<SEPerformanceGuideProps> = ({ onBack, startAbout = false }) => {
  const [activeStep, setActiveStep] = useState(0);
  const [showAbout, setShowAbout] = useState(startAbout);
  const stepRefs = useRef<(HTMLDivElement | null)[]>([]);
  const { sePerformance } = useInfraLens();
  ...
  }, [sePerformance, showAbout]);

  const handlePrint = () => { window.print(); };

  const handleExportMarkdown = () => {
    const content = `# SE PERFORMANCE PROTOCOL - VERSION 1.0
## BIOLOGICAL PERFORMANCE SPECIFICATION
...
### 2. CORE MODULES
${sePerformance.map((s, i) => `- ${i+1}. **${s.title}**: ${s.description} (${s.actionLabel}: ${s.actionText})`).join('\n')}
...
[END OF CANONICAL SE_PERFORMANCE SPEC]`;

    const blob = new Blob([content], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'se-performance-protocol.md';
        ...
        <aside className="no-print w-full md:w-72 border-b md:border-b-0 md:border-r border-border bg-page-bg flex flex-col shrink-0 z-30">
           <div className="p-8 flex items-center gap-3">
              ...
              <div className="flex flex-col">
                  ...
                  <span className="tool-label mt-1">v1.0</span>
              </div>
           </div>
           <nav className="flex-1 px-4 space-y-2 pt-4">
              <div className="p-6 bg-emerald-950/10 border border-emerald-500/20 rounded-2xl mb-4">
                  <h3 className="text-emerald-400 font-bold text-sm mb-2 flex items-center gap-2"><Fingerprint size={16}/> SE Performance</h3>
                  <p className="text-xs text-secondary leading-relaxed">Detailed protocols for physiological and cognitive optimization. Intended for daily execution.</p>
              </div>
           ...
           <div className="p-6 border-t border-border">
              <button onClick={onBack} className="text-xs text-secondary hover:text-primary flex items-center gap-2 uppercase tracking-widest"><ArrowLeft size={12}/> Return to About</button>
           </div>
        </aside>
        ...
        <main className="no-print flex-1 overflow-y-auto bg-[#09090b] relative p-8 md:p-20">
           ...
           <div className="max-w-4xl mx-auto space-y-20 animate-fade-in pb-32">
              <section className="space-y-6">
                  <h1 className="text-6xl md:text-8xl font-serif font-bold tracking-tighter">SE Performance</h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="p-8 bg-card-bg/50 border border-border rounded-3xl">
                      <h3 className="font-bold text-xl mb-4 text-primary">System Architecture</h3>
                      <p className="text-secondary text-sm leading-relaxed">The SE Performance Guide treats the body as a hierarchical state machine. Rhythms (Circadian) govern energy levels, which govern focus (Ultradian), which determines cognitive output.</p>
                  </div>
                  <div className="p-8 bg-card-bg/50 border border-border rounded-3xl">
                      <h3 className="font-bold text-xl mb-4 text-primary">Execution Layers</h3>
                      <ul className="space-y-3 text-xs font-mono text-secondary">
                          <li className="flex justify-between border-b border-border pb-1"><span>L1: IDENTITY</span><span>Values & Baseline</span></li>
                          <li className="flex justify-between border-b border-border pb-1"><span>L2: BIOLOGICAL</span><span>Light, Fuel, Sleep</span></li>
                          <li className="flex justify-between border-b border-border pb-1"><span>L3: COGNITIVE</span><span>Work, Synthesis</span></li>
                      </ul>
                  </div>
              </div>
           </div>
        ...
        <div className="print-only hidden mx-auto bg-white text-black p-0 overflow-visible" style={{ width: '210mm' }}>
           <div className="p-16 border-b border-gray-100 min-h-[297mm] flex flex-col relative print-break-after">
              ...
              <header className="mb-20 border-b-8 border-black pb-8">
                  <h1 className="text-6xl font-serif font-black uppercase tracking-tighter mb-4">SE Performance Specification</h1>
              ...
              <footer className="mt-auto pt-8 border-t border-gray-200 text-center font-mono text-[8px] uppercase text-gray-400 tracking-[0.5em]">SE Performance // Section I // Core v1.0</footer>
           </div>
           ...
           <div className="p-16 border-b border-gray-100 min-h-[297mm] flex flex-col relative print-break-after">
              ...
              <footer className="mt-auto pt-8 border-t border-gray-200 text-center font-mono text-[8px] uppercase text-gray-400 tracking-[0.5em]">SE Performance // Section II // Execution Map</footer>
           </div>
...
{`# SE PERFORMANCE PROTOCOL - VERSION 1.0
## BIOLOGICAL PERFORMANCE SPECIFICATION
...
[END OF CANONICAL SE_PERFORMANCE SPEC]`}
  ...
  return (
    <div className="bg-page-bg text-primary font-sans selection:bg-indigo-500/30 overflow-x-hidden min-h-screen">
      <nav className="fixed top-0 left-0 w-full z-50 p-6 flex justify-between items-center mix-blend-difference pointer-events-none">
        <div className="flex items-center gap-6 pointer-events-auto">
            <button onClick={onBack} className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest hover:text-indigo-400 transition-colors text-primary">
            ...
            <Info size={14} /> About Performance Guide
            </button>
        </div>
        <span className="font-mono text-xs text-white/50">v1.0 Core</span>
      </nav>
      <div className="fixed top-0 left-0 h-1 bg-card-bg w-full z-50">
      ...
      <div className="relative flex flex-col md:flex-row">
          ...
          <div className="hidden md:block w-1/2 h-screen sticky top-0 bg-page-bg z-10 border-l border-border shadow-2xl">
