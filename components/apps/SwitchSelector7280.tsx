...
export const SwitchSelector7280: React.FC<SwitchSelector7280Props> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans flex flex-col md:flex-row overflow-hidden selection:bg-cyan-500/30">
         ...
         <div className="p-8 border-b border-zinc-900">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest mt-0.5">R3/R3A Matrix v4.1</div>
                </div>
            </div>
         </div>
         ...
         <div className="p-8 space-y-8 flex-1">
            <section className="space-y-4">
                ...
                <div className="relative group">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-cyan-500 transition-colors" size={14} />
                    <input 
                      value={search}
                      onChange={e => setSearch(e.target.value)}
                      placeholder="SKU, Use Case, Port Speed..."
                      className="w-full bg-zinc-900 border border-zinc-800 rounded-xl py-3 pl-9 pr-4 text-xs focus:border-cyan-500 outline-none transition-all placeholder-zinc-700"
                    />
                </div>
            </section>

            <section className="space-y-4">
                <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.3em]">Port Speeds</h3>
                <div className="grid grid-cols-2 gap-2">
                    {allSpeeds.map(speed => (
                        <button 
                            key={speed}
                            onClick={() => setFilterSpeed(filterSpeed === speed ? null : speed)}
                            className={`p-2 rounded-xl border text-[10px] font-bold transition-all text-center flex items-center justify-center gap-2 ${filterSpeed === speed ? 'bg-cyan-500/10 border-cyan-500 text-cyan-400' : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:border-zinc-700'}`}
                        >
      ...
      <main className="flex-1 overflow-y-auto bg-[#09090b] relative flex flex-col">
         ...
         <div className="flex-1 p-8 md:p-16 relative z-10 max-w-7xl mx-auto w-full">
            <header className="mb-12 border-b border-zinc-900 pb-12">
                ...
                <h2 className="text-5xl md:text-7xl font-serif font-bold tracking-tighter mb-6 text-white leading-none">The R3 Advantage</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {filteredModels.length > 0 ? filteredModels.map((model) => {
                  ...
                  return (
                    <div key={model.id} className="group p-8 bg-zinc-900 border border-zinc-800 rounded-[2.5rem] hover:border-cyan-500/50 transition-all flex flex-col shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-8">
                            <div>
                                ...
                                <h3 className="text-4xl font-serif font-bold text-white leading-tight mb-2 tracking-tight group-hover:text-cyan-400 transition-colors">{model.model}</h3>
                            ...
                            <div className="p-4 bg-zinc-950 border border-zinc-800 rounded-[1.5rem] group-hover:scale-110 transition-transform shadow-inner text-cyan-400">
                                <Cpu size={24} />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-8">
                            ...
                            <div className="p-5 bg-zinc-950/30 rounded-3xl border border-zinc-800 group-hover:border-zinc-700 transition-colors">
                                <span className="text-[8px] font-mono text-zinc-600 uppercase tracking-widest block mb-2">VOQ Buffer</span>
                                <span className="text-lg font-bold text-cyan-400">{model.buffer}</span>
                            </div>
                            <div className="p-5 bg-zinc-950/30 rounded-3xl border border-zinc-800 group-hover:border-zinc-700 transition-colors">
                                <span className="text-[8px] font-mono text-zinc-600 uppercase tracking-widest block mb-2">Packet Rate</span>
                                <span className="text-sm font-bold text-zinc-200">{model.pps}</span>
                            </div>
                            <div className="p-5 bg-zinc-950/30 rounded-3xl border border-zinc-800 group-hover:border-zinc-700 transition-colors">
                    ...
                    <div className="col-span-full py-32 flex flex-col items-center justify-center text-center space-y-6 opacity-30">
                        ...
                        <div className="space-y-2">
                            <h3 className="text-2xl font-serif italic text-white">Spec combination not found.</h3>
                            <p className="text-sm text-zinc-500">Try loosening your search or speed constraints.</p>
                        </div>
                    </div>
                )}
            </div>
